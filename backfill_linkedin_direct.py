"""Fetch remaining LinkedIn job descriptions directly from job view pages."""
...
with db() as conn:
    rows = conn.execute(
        "SELECT id, url FROM jobs WHERE source='linkedin' AND status != 'archived' "
        "AND (description IS NULL OR description='')"
    ).fetchall()

print(f"LinkedIn jobs still needing descriptions: {len(rows)}")
done = 0
errors = 0
        ...
        try:
            ...
            el = (
                soup.find("div", class_=lambda c: c and any(
                    x in c for x in ["description__text", "show-more-less-html"]
                ))
                or soup.find("div", attrs={"class": lambda c: c and "description" in str(c).lower()})
            )
            if el:
